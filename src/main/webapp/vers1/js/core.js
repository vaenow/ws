...
Core.init = function(){
	...
	//绑定快捷方式点击事件
	ul.find('li').live('click',function(){Core.create($(this));});
	//绑定任务栏点击事件
	$('.task-window li').live('click',function(){Core.taskwindow($(this));}).live('contextmenu',function(){
		//展示自定义右键菜单
	...
	//绑定窗口点击事件
	$('.window-container').live('click',function(){Core.container($(this));});
};
...
Core.create = function(obj,opt){
	if(obj==""){
		...
		var options = {
			num		:jsonsc['data'][sc]['id'],
			title	:jsonsc['data'][sc]['iconName'],
			imgsrc	:jsonsc['data'][sc]['iconUrl'],
			url		:jsonsc['data'][sc]['url'],
			width	:jsonsc['data'][sc]['width'],
			height	:jsonsc['data'][sc]['height'],
			resize	:true
		};
	}
...
Core.handle = function(obj){
	...
	//最小化
	obj.find(".ha-hide").bind("click",function(e){
		//阻止冒泡
		e.stopPropagation();
		obj.hide();
		//改变任务栏样式
...
Core.bindWindowMove = function(obj){
	obj.find(".title-bar").unbind("mousedown").bind("mousedown",function(e){
		//改变窗口为选中样式
		$('.window-container').removeClass('window-current');
		obj.addClass('window-current').css({'z-index':Core.config.createIndexid});
		Core.config.createIndexid += 1;
